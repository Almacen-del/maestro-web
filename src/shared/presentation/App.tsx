...
interface AppProps {
  ...
  readonly reportPlatform?: ReportPlatform;
}
...
export function App({repository, reportPlatform}: AppProps) {
  ...
  const [nurseryModule, setNurseryModule] = useState<NurseryModule>("HOME");
  ...
  return (
    <main className={user ? `app-shell nursery-shell${sidebarCollapsed ? " nursery-shell--collapsed" : ""}` : "app-shell"}>
      ...
      {user && <NurserySidebar user={user} collapsed={sidebarCollapsed} onToggle={() => setSidebarCollapsed((value) => !value)} activeModule={nurseryModule} onModule={setNurseryModule} onSignOut={() => void handleSignOut()} />}
      ...
      {restoringSession ? <section className="nursery-session-loading" role="status" aria-live="polite"><img src="/logo-arles.jpeg" alt="Arles" /><h1>Vivero</h1><p>Restaurando sesión…</p></section> : !user ? (
        ...
        nurseryModule === "HOME" ? <NurseryHomeSection repository={repository} /> : nurseryModule === "DAILY" ? <DailyProcessesSection /> : nurseryModule === "MONITORING" ? <MonitoringSection /> : nurseryModule === "APPLICATIONS" ? <ApplicationsSection /> : nurseryModule === "INVENTORY" ? <InventorySection repository={repository} /> : <PendingNurseryModule module={nurseryModule} />
